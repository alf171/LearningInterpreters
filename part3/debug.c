...
static int constantInstruction(const char *name, Chunk *chunk, int offset) {
  ...
  // Never crash on bad bytecode: show a readable error line instead.
  if (constant >= chunk->constants.count) {
    printf("%04d %-16s <bad constant index %u>\n", offset,
           name ? name : "<NULL>", constant);
    return offset + 2;
  }

  printf("%04d %-16s %4d '", offset, name ? name : "<NULL>", constant);
...
int disassembleInstruction(Chunk *chunk, int offset) {
  ...
  switch (instruction) {
    ...
    case OP_GET_SUPER:
      return simpleInstruction("OP_GET_SIMPLE", offset);
    case OP_GREATER:
